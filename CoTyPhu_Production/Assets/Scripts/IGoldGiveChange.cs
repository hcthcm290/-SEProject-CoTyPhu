...
public interface IGoldGiveChange
{
	//  Properties ------------------------------------
	float goldGiveChange { get; set; }

	//  Methods ---------------------------------------
	float GetGoldGiveChange(float basePrice, float delta); //return về một tổng delta cũ và delta mới (delta là số tiền thay đổi)
